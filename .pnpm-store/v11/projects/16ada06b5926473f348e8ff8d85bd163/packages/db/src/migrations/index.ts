...
export const migrations: Migration[] = [
  ...
  },
  {
    ...
    up: (db) => {
      ...
      if (!hasColumn('room', 'corridor_id')) db.exec(`ALTER TABLE room ADD COLUMN corridor_id TEXT;`);
      if (!hasColumn('room', 'room_type_id')) db.exec(`ALTER TABLE room ADD COLUMN room_type_id TEXT;`);
      if (!hasColumn('room', 'layout_side')) db.exec(`ALTER TABLE room ADD COLUMN layout_side TEXT NOT NULL DEFAULT 'none';`);
      if (!hasColumn('room', 'sort_order')) db.exec(`ALTER TABLE room ADD COLUMN sort_order INTEGER NOT NULL DEFAULT 0;`);
