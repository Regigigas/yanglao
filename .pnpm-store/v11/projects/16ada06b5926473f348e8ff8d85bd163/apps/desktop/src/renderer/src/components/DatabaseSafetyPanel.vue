...
<template>
  <div class="data-safety-tools">
    <section class="data-safety-tool">
      <span class="data-safety-icon"><DatabaseConfig theme="outline" :size="20" :stroke-width="3" /></span>
      <div>
      ...
      <NButton type="primary" :loading="backupCreating" @click="createBackup">
        <template #icon><Save class="data-button-icon" theme="outline" :size="16" :stroke-width="3" /></template>
        立即备份
      </NButton>
    </section>

    <section class="data-safety-tool">
      <span class="data-safety-icon integrity"><DatabaseSuccess theme="outline" :size="20" :stroke-width="3" /></span>
      <div>
      ...
      <NButton :loading="integrityChecking" @click="checkIntegrity">
        <template #icon><CheckOne class="data-button-icon" theme="outline" :size="16" :stroke-width="3" /></template>
        运行检查
      </NButton>
    </section>

    <section class="data-safety-tool local-sync-tool">
      <span class="data-safety-icon sync"><DatabaseSync theme="outline" :size="20" :stroke-width="3" /></span>
      <div>
      ...
      <NButton type="info" secondary :loading="localSyncing" @click="syncLocalFile">
        <template #icon><DocAdd class="data-button-icon" theme="outline" :size="16" :stroke-width="3" /></template>
        选择数据文件
      </NButton>
    </section>
  </div>
  ...
  <div class="backup-heading">
    ...
    <NSpace>
      <NButton quaternary title="刷新备份历史" @click="loadBackups">
        <template #icon><Refresh class="data-button-icon" theme="outline" :size="16" :stroke-width="3" /></template>
        刷新
      </NButton>
      <NButton quaternary title="打开备份目录" @click="openBackupDirectory">
        <template #icon><FolderOpen class="data-button-icon" theme="outline" :size="16" :stroke-width="3" /></template>
        打开目录
      </NButton>
    </NSpace>
  </div>
        ...
        <tr v-for="backup in backups" :key="backup.name">
          ...
          <td>
            <NSpace :wrap="false" size="small">
              ...
              >
                <template #icon><Reload class="data-button-icon" theme="outline" :size="16" :stroke-width="3" /></template>
                恢复
              </NButton>
              <NButton
                quaternary
              ...
              >
                <template #icon><Download class="data-button-icon" theme="outline" :size="16" :stroke-width="3" /></template>
                导出
              </NButton>
            </NSpace>
          </td>
