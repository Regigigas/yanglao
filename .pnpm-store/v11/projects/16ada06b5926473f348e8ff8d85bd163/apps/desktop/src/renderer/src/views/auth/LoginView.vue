<script setup lang="ts">
  ...
  import { Lock, User } from '@icon-park/vue-next'
      ...
      <NForm :model="form" label-placement="left" label-width="0">
        <NFormItem>
          <NInput v-model:value="form.username" placeholder="用户名" size="large" @keydown.enter="handleLogin">
            <template #prefix><User class="login-input-icon" theme="outline" :size="18" :stroke-width="3" /></template>
          </NInput>
        </NFormItem>
        <NFormItem>
          <NInput
          ...
          >
            <template #prefix><Lock class="login-input-icon" theme="outline" :size="18" :stroke-width="3" /></template>
          </NInput>
        </NFormItem>
